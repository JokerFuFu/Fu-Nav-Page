...
- **背景壁纸**：内置多组 AI 生成主题壁纸（深浅色各配），支持 Bing 每日一图、在线壁纸源、本地上传。
- **内置图标库**：图标编辑「在线」里内置上百个常用服务/homelab 图标，可搜索点选，免 favicon 抓取失败。
- **易操作**：右上/侧栏 `＋` 加站；卡片编辑（链接、名称、备注、图标、所属分组、是否首页常用）；编辑模式锁定/解锁；一键导入浏览器书签；导入/导出备份。
- **内网在线状态**：对 `192.168.1.x` 这类内网服务 best-effort 探测，确认在线才亮绿点。
- **搜索**：站内过滤 + 引擎切换（Bing/Google/百度/DuckDuckGo），快捷键 `/` 聚焦；深/浅色主题。
- **内置演示配置**：首次安装即见完整示例（分组、文件夹、常用大卡、监控卡），可一键导入自己的浏览器书签后替换。

更多进阶能力——WebDAV 自托管云同步、浏览器书签双向同步、Infinity 备份导入、硬件监控卡、⌘K 命令面板、工作区与归档、本机伴随服务——见下文「进阶功能」。
...
| `v1.hitokoto.cn` | 一言（历史功能，当前版本已下线，该权限将在后续版本清理） |
...
零依赖 Node 本机服务，**只监听 `127.0.0.1`**，补齐扩展沙箱做不到的能力：内网服务真实在线探测、本机硬件监控（免装 Glances）、提醒事项/日历/AI 日报小组件。不安装不影响任何主功能，取不到数据时相关小组件自动隐藏。详见 [agent/README.md](agent/README.md)。
...
分组可设「工作区」标签（运维/影音/工作…），侧栏自动派生 tab 过滤；低频分组可**归档**到侧栏折叠区，主列表只留高频，治书签膨胀。
...
可选的本机 Node 服务，仅监听 `127.0.0.1`，为扩展补齐内网探测、本机硬件监控、提醒/日历等能力。完全可选，不装不影响主功能。
...
background.js          # Service Worker：已收藏页面的工具栏 ✓ 角标
...
shared/
  ...
  agent.js             # 本机伴随服务对接
  background.js / bg-presets.js / bg-storage.js / accent-presets.js   # 背景壁纸与主题强调色
